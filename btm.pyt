...
class Toolbox(object):
    ...
    def __init__(self):
        ...
        self.tools = [
            ...
            mcvc,               # runs selected btm functions
...
class arcchordratio(object):
    ...
    def updateMessages(self, parameters):
        ...
        save_tins = parameters[2].valueAsText
        ...
        if aoi:
            ...
            aoi_path = aoi_desc.catalogPath
            if '.gdb' in aoi_path and save_tins == 'true' and \
                    out_workspace is None:
                # They've selected a FileGDB data source, asked
                # for TINs to be written. By default the code will try
                # to write the TINs to the same location as the AOI,
                # this will fail. Force them to set a workspace in this case.
                aoi_msg = ("By default, this tool writes the TINs to the "
                           "same location as the Area of Interest polygons. "
                           "TINs have been selected to be created, but can't "
                           "be created within a File Geodatabase. Please "
                           "select a directory for the Output Workspace "
                           "parameter.")
                parameters[1].addErrorMessage(aoi_msg)

        if out_workspace is not None and '.gdb' in out_workspace.lower() \
                and save_tins == 'true':
            gdb_tin_msg = ("Can't write TINs to a File Geodatabase, "
                           "please choose a directory for the "
                           "Output Workspace.")
            parameters[3].addErrorMessage(gdb_tin_msg)
...
class mcvc (object):
    ...
    def __init__(self):
        self.label = 'Mean Complexity Value Collector'
        self.canRunInBackground = False
    ...
    def getParameterInfo(self):
        ...
        # Input site geodatabase Folder
        siteData = arcpy.Parameter()
        siteData.name = 'Site Geodatabase'
        siteData.displayName = 'Site Geodatabase'
        siteData.parameterType = 'Required'
        siteData.direction = 'Input'
        siteData.datatype = dt.format('Workspace')

        # Output .csv
        excel_doc = arcpy.Parameter()
        excel_doc.name = 'Output Spreadsheet'
        excel_doc.displayName = 'Output Spreadsheet (.csv)'
        excel_doc.parameterType = 'Required'
        ...
        clipDEM.displayName = 'Clip DEMs?'
        clipDEM.parameterType = 'Optional'
        clipDEM.direction = 'Input'
        clipDEM.datatype = dt.format('Boolean')
        clipDEM.value = 'False'
        
        # Input Raster
        in_dem = arcpy.Parameter()
        in_dem.name = 'Input DEM'
        in_dem.displayName = 'Input DEM'
        in_dem.parameterType = 'Optional'
        in_dem.direction = 'Input'
        in_dem.datatype = dt.format('File')
        
        # Clip Ortho
        clipOrtho = arcpy.Parameter()
        clipOrtho.name = 'Clip Ortho'
        clipOrtho.displayName = 'Clip Ortho?'
        clipOrtho.parameterType = 'Optional'
        clipOrtho.direction = 'Input'
        clipOrtho.datatype = dt.format('Boolean')
        clipOrtho.value = 'False'
        
        # Input Raster
        in_ortho = arcpy.Parameter()
        in_ortho.name = 'Input Ortho'
        in_ortho.displayName = 'Input Ortho'
        in_ortho.parameterType = 'Optional'
